    ...
    <div class="overflow-x-hidden">
        <main class="pt-6 px-6">
            ...
            <div class="bg-white rounded-lg shadow-md p-8 my-8">
                <!-- OPTION -->
                <form method="POST" action="{{ url('administrator/data-mapel/filter-semester') }}">
                    @csrf
                    <div class="flex space-x-4 items-center pb-8">
                        ...
                        <div class="flex-none md:w-1/5">
                            <div class="relative">
                                ...
                                <select type="number" name="sem" value=""  class="block appearance-none w-full bg-grey-lighter border border-grey-lighter text-grey-darker py-3 px-4 pr-8 rounded" id="grid-state">
                                    @foreach($filters_semester as $filter)
                ...
                <form method="POST" action="{{ url('administrator/data-mapel/filter-status') }}">
                    @csrf
                    <div class="flex space-x-4 items-center pb-8">
                        <div class="flex-none w-36">
                        ...
                        <div class="flex-none md:w-1/5">
                            <div class="relative">
                                ...
                                <select type="text" name="status_course" value="" class="block appearance-none w-full bg-grey-lighter border border-grey-lighter text-grey-darker py-3 px-4 pr-8 rounded" id="grid-state">
                                    @foreach($status as $filter)
                                    <option value="{{ $filter->status_course }}">{{ $filter->status_course }}</option>
                                    @endforeach
                                    @foreach($filters_status as $filter)
                                    <option value="{{ $filter->status_course }}">{{ $filter->status_course }}</option>
                                    @endforeach
                                </select>
                            </div>
                ...
                @elseif($hapus = Session::get('update'))
