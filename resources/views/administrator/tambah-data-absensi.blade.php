...
<div class="w-full flex flex-col h-screen overflow-y-hidden">
    <div class="overflow-x-hidden">
        <main class="pt-6 px-6">
            <h1 class="text-3xl text-black pb-2 mt-2">Data Absensi</h1>
            <div class="bg-white rounded-lg shadow-md p-8 my-8">
                <div class="p-4">
                    <a href="{{ url('administrator/data-absensi/list') }}/{{ $idSantri }}" class="button flex items-center border border-black-500 text-black-500 rounded-sm py-2.5 px-5 w-32 hover:bg-blue-700 hover:text-white">
                ...
                <form method="POST" action="{{ url('administrator/data-absensi/create') }}">
                    ...
                    <div class="pb-8">
                        ...
                        <div class="md:grid md:grid-cols-2 hover:bg-gray-50 md:space-y-0 px-4 py-2 space-y-1">
                            ...
                            <div class="relative z-0 w-full mb-5">
                                <select type="text" name="year" required onclick="this.setAttribute('value', this.value);" class="pt-3 pb-2 px-3 block w-full mt-0 bg-transparent border-0 border-b-2 appearance-none z-1 focus:outline-none focus:ring-0 focus:border-black border-gray-200">
                                    <option selected value="{{ date('Y')-1 . '/' . date('Y') }}">{{ date('Y')-1 . '/' . date('Y') }}</option>
                        ...
                        <div class="md:grid md:grid-cols-2 hover:bg-gray-50 md:space-y-0 px-4 py-2 space-y-1">
                            ...
                            <div class="relative z-0 w-full mb-5">
                                <input type="number" name="attendance_mdnu" placeholder="1-100%" min="0" max="100" required class="pt-3 pb-2 px-3 block w-full mt-0 bg-transparent border-0 border-b-2 appearance-none focus:outline-none focus:ring-0 focus:border-black border-gray-200" />
                            </div>
                        </div>
                        <div class="md:grid md:grid-cols-2 hover:bg-gray-50 md:space-y-0 px-4 py-2 space-y-1">
                            <p class="self-center text-gray-600">Presentase  Absensi Asrama</p>
                            <div class="relative z-0 w-full mb-5">
                                <input type="number" name="attendance_asrama" placeholder="0-100%" min="0" max="100" required class="pt-3 pb-2 px-3 block w-full mt-0 bg-transparent border-0 border-b-2 appearance-none focus:outline-none focus:ring-0 focus:border-black border-gray-200" />
                            </div>
                        </div>
                    </div>
